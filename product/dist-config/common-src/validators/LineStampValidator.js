...
exports.LineStampValidator = void 0;
/**
 * LINEスタンプの規約に即しているかを検証するバリデーターです。
 */
var LineStampValidator = /** @class */ (function () {
    function LineStampValidator() {
    }
    LineStampValidator.validate = function (stat, options, localeData) {
        var validateArr = [];
        ...
        if (LineStampValidator.validateFrameMaxSize(options) === false) {
            ...
            validateArr.push(localeData.VALIDATE_maxSize.split('${1}')
                .join(val1 + '')
                .split('${2}')
                .join(val2 + ''));
        }
        if (LineStampValidator.validateFrameMinSize(options) === false) {
            var val1 = options.imageInfo.width;
            var val2 = options.imageInfo.height;
            validateArr.push(localeData.VALIDATE_minSize.split('${1}')
                .join(val1 + '')
                .split('${2}')
                .join(val2 + ''));
        }
